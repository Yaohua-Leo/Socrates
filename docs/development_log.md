...
Socrates 已从最初的 Python CLI skeleton 推进到可运行的本地数学学习 CLI 原型。v0.1/v0.2 的确定性学习闭环、Reference KB 与 Obsidian 笔记沉淀已经落地；当前实现已推进到 v0.60 real-use validation runbook and development pause，覆盖 v0.59-alpha 的 deterministic product readiness audit、returning-learner deterministic MVP lifecycle canary evidence、inspectable deterministic MVP lifecycle canary artifact bundle、structured project-status JSON、structured study-brief-status JSON、structured learning-queue JSON、structured study-dashboard JSON、dry-run-previewable structured misconception resolver writer JSON、dry-run-previewable structured review-adjust-plan writer JSON、dry-run-previewable structured review-exercise writer JSON、dry-run-previewable structured review-schedule repair writer JSON、structured review-schedule writer JSON、structured due-review JSON、structured learning-mastery JSON、structured misconception-ledger JSON 与 targeted limit-controlled dry-run-previewable structured batch-refreshable commands-output command-summarized filterable readiness-counted multi-project resume index 层。

当前分支相对早期主线已有大量功能提交。最近一组工作从 Reference KB reader gate 和 v0.3 LLM draft 边界继续推进到 v0.60 real-use validation pause：生成 artifact 不能只存在，还必须结构有效、来源可追溯，并且完成 workflow 后能被已有 study brief、dashboard、resume returning-learner surfaces 重新进入；选择下一条大产品路线前，最终目标对应的 capability evidence、gap 和 decision point 必须清楚可见；在继续 OCR/PDF、autonomous LLM tutoring 或 UI/plugin 前，Leo 需要先用一个真实学习主题跑完 v0.60 runbook 并反馈真实摩擦点。
...
## v0.40 Review misconceptions JSON alpha

- Added `--json` to `python -m socrates review misconceptions`.
- JSON output reuses `list_misconceptions(context, status=...)`, so Markdown and JSON views share the same filtered misconception rows and deterministic ordering.
- The payload records `schema_version`, `quality_boundary: deterministic_misconception_review`, `project`, `status_filter`, returned-row counts, and structured misconception rows with analysis, repair suggestions, and follow-up exercises.
- `--status active --json` returns only active rows and does not mutate `learning_state.json`, `mistake_bank.md`, note drafts, reports, or project logs.
- Safety boundary: v0.40 misconception JSON is a read-only ledger view. It is not a resolver, note generator, tutor, score, report refresh, LLM call, or learning-state mutation.

Verification evidence:
  - `python -m unittest tests.test_state_eval` failed during RED because `review misconceptions --json` was not registered.
  - `python -m unittest tests.test_state_eval` passed with 42 tests OK after implementation.
  - `python -m unittest tests.test_state_eval tests.test_note_review_export tests.test_status_quality_summary tests.test_learning_queue tests.test_reports` passed with 156 tests OK.
  - `python -m unittest discover -s tests` passed with 439 tests OK and 1 skipped.
  - `python -m compileall socrates` passed.
  - `powershell -ExecutionPolicy Bypass -File scripts\check.ps1` passed with 439 tests OK and 1 skipped.
  - `bash scripts/check.sh` passed with 439 tests OK and 1 skipped; WSL emitted localhost text and a `scripts/check.ps1` line-ending warning, but the script exit code was 0.

## v0.41 Review mastery JSON alpha

- Added `--json` to `python -m socrates review mastery`.
- JSON output reuses `list_learning_scores(context, score_type=..., status=..., threshold=...)`, so Markdown and JSON views share the same filtered concept/proof-skill score rows and deterministic ordering.
- The payload records `schema_version`, `quality_boundary: deterministic_learning_mastery_review`, `project`, `kind_filter`, `status_filter`, `threshold`, returned-row counts, and structured score rows.
- `--kind concept --status weak --threshold 0.8 --json` returns only weak concept rows under that threshold and does not mutate `learning_state.json`, review schedules, reports, note drafts, or project logs.
- Safety boundary: v0.41 mastery JSON is a read-only score ledger view. It is not a scheduler, planner, tutor, score writer, report refresh, LLM call, or learning-state mutation.

Verification evidence:
  - `python -m unittest tests.test_state_eval` failed during RED because `review mastery --json` was not registered.
  - `python -m unittest tests.test_state_eval` passed with 44 tests OK after implementation.
  - `python -m unittest tests.test_state_eval tests.test_status_quality_summary tests.test_learning_queue tests.test_reports` passed with 137 tests OK.
  - `python -m unittest discover -s tests` passed with 441 tests OK and 1 skipped.
  - `python -m compileall socrates` passed.
...
## v0.43 Review schedule JSON alpha

- Added `--json` to `python -m socrates review schedule`.
- JSON output still calls `build_review_schedule(...)`, so it performs the same scheduler write as Markdown mode before serializing the persisted schedule rows.
- The payload records `schema_version`, `quality_boundary: deterministic_review_schedule_writer`, `project`, `as_of`, `threshold`, scheduled row count, schedule path, and structured scheduled-review rows.
- Empty schedules return `scheduled_count: 0` and still write `02_learning_plan/review_schedule.md`.
- Safety boundary: v0.43 schedule JSON is a writer-result payload. It is not a dry-run, read-only ledger, tutor, planner, repair command, exercise generator, report refresh, LLM call, or extra learning-state truth beyond the existing scheduler write.

Verification evidence:
  - `python -m unittest tests.test_state_eval` failed during RED because `review schedule --json` was not registered.
  - `python -m unittest tests.test_state_eval` passed with 48 tests OK after implementation.
  - `python -m unittest tests.test_state_eval tests.test_status_quality_summary tests.test_learning_queue tests.test_reports` passed with 141 tests OK.
  - `python -m unittest discover -s tests` passed with 445 tests OK and 1 skipped.
  - `python -m compileall socrates` passed.
  - `powershell -ExecutionPolicy Bypass -File scripts\check.ps1` passed with 445 tests OK and 1 skipped.
  - `bash scripts/check.sh` passed with 445 tests OK and 1 skipped; WSL emitted localhost text, but the script exit code was 0.

## v0.44 Review repair-schedule JSON alpha

- Added `--json` to `python -m socrates review repair-schedule`.
- JSON output still calls `repair_review_schedule(...)`, so it performs the same repair write as prose mode before serializing the persisted schedule rows.
- The payload records `schema_version`, `quality_boundary: deterministic_review_schedule_repair_writer`, `project`, `as_of`, repaired count, schedule path, and structured scheduled-review rows.
- No-op repairs return `repaired_count: 0` and still return the current post-write schedule rows.
- Safety boundary: v0.44 repair JSON is a repair-writer result payload. It is not a dry-run, read-only ledger, scheduler, tutor, planner, exercise generator, report refresh, LLM call, or extra learning-state truth beyond the existing repair write.

Verification evidence:
  - `python -m unittest tests.test_state_eval` failed during RED because `review repair-schedule --json` was not registered.
  - `python -m unittest tests.test_state_eval` passed with 50 tests OK after implementation.
  - `python -m unittest tests.test_state_eval tests.test_status_quality_summary tests.test_learning_queue tests.test_reports` passed with 143 tests OK.
  - `python -m unittest discover -s tests` passed with 447 tests OK and 1 skipped.
  - `python -m compileall socrates` passed.
...
## v0.46 Review exercises JSON alpha

- Added `--json` to `python -m socrates review exercises`.
- JSON output still calls `generate_targeted_review_exercise_drafts(...)`, so it performs the same targeted exercise draft writes as prose mode before serializing the returned rows.
- The payload records `schema_version`, `quality_boundary: deterministic_review_exercise_writer`, `project`, `due_by`, `priority_filter`, generated count, and structured generated exercise rows.
- `--due-by ... --priority high --json` preserves the same due-date and priority filters as prose mode.
- Safety boundary: v0.46 review exercises JSON is a writer-result payload. It is not a dry-run, read-only ledger, scheduler, planner, tutor, report refresh, LLM call, exercise validation, approval, grading, or learning-state mutation.

Verification evidence:
  - `python -m unittest tests.test_state_eval` failed during RED because `review exercises --json` was not registered.
  - `python -m unittest tests.test_state_eval` passed with 54 tests OK after implementation.
  - `python -m unittest tests.test_state_eval tests.test_status_quality_summary tests.test_learning_queue tests.test_reports` passed with 147 tests OK.
  - `python -m unittest discover -s tests` passed with 451 tests OK and 1 skipped.
  - `python -m compileall socrates` passed.
  - `git diff --check` passed.
  - `powershell -ExecutionPolicy Bypass -File scripts\check.ps1` passed with 451 tests OK and 1 skipped.
  - `bash scripts/check.sh` passed with 451 tests OK and 1 skipped; WSL emitted localhost text and a `scripts/check.ps1` line-ending warning, but the script exit code was 0.

## v0.47 Review exercises dry-run alpha

- Added `--dry-run` to `python -m socrates review exercises`.
- Dry-run mode uses the same targeted review exercise selection as the writer but does not create generated exercise draft files.
- `--dry-run --json` emits `quality_boundary: deterministic_review_exercise_preview`, `dry_run: true`, filter metadata, generated count, and preview generated-exercise rows.
- Prose dry-run output reports how many targeted review exercises would be generated and lists the preview rows.
- Safety boundary: v0.47 review exercises dry-run is a no-write preview. It is not a writer, read-only ledger, scheduler, planner, tutor, report refresh, LLM call, exercise validation, approval, grading, or learning-state mutation.

Verification evidence:
  - `python -m unittest tests.test_state_eval` failed during RED because `review exercises --dry-run` was not registered.
  - `python -m unittest tests.test_state_eval` passed with 56 tests OK after implementation.
  - `python -m unittest tests.test_state_eval tests.test_status_quality_summary tests.test_learning_queue tests.test_reports` passed with 149 tests OK.
  - `python -m unittest discover -s tests` passed with 453 tests OK and 1 skipped.
  - `python -m compileall socrates` passed.
  - `git diff --check` passed.
  - `powershell -ExecutionPolicy Bypass -File scripts\check.ps1` passed with 453 tests OK and 1 skipped.
  - `bash scripts/check.sh` passed with 453 tests OK and 1 skipped; WSL emitted localhost text and a `scripts/check.ps1` line-ending warning, but the script exit code was 0.

## v0.48 Review adjust-plan JSON alpha

- Added `--json` to `python -m socrates review adjust-plan`.
- JSON output still calls `adjust_short_term_plan_from_review_schedule(...)`, so it performs the same short-term-plan rewrite as prose mode before serializing persisted review-adjustment rows.
- The payload records `schema_version`, `quality_boundary: deterministic_review_adjust_plan_writer`, `project`, `short_term_plan_path`, adjustment count, and structured review-adjustment rows.
- Corrupt `learning_state.json` handling remains clean in JSON mode and does not rewrite the short-term plan.
- Safety boundary: v0.48 review adjust-plan JSON is a writer-result payload. It is not a dry-run, scheduler, exercise generator, tutor, report refresh, LLM call, plan approval, or learning-state mutation.

Verification evidence:
  - `python -m unittest tests.test_learning_plan` failed during RED because `review adjust-plan --json` was not registered.
  - `python -m unittest tests.test_learning_plan` passed with 14 tests OK after implementation.
  - `python -m unittest tests.test_learning_plan tests.test_state_eval tests.test_status_quality_summary tests.test_learning_queue tests.test_reports` passed with 163 tests OK.
  - `python -m unittest discover -s tests` passed with 455 tests OK and 1 skipped.
  - `python -m compileall socrates` passed.
  - `git diff --check` passed.
  - `powershell -ExecutionPolicy Bypass -File scripts\check.ps1` passed with 455 tests OK and 1 skipped.
  - `bash scripts/check.sh` passed with 455 tests OK and 1 skipped; WSL emitted localhost text and a `scripts/check.ps1` line-ending warning, but the script exit code was 0.

## v0.49 Review adjust-plan dry-run alpha

- Added `--dry-run` to `python -m socrates review adjust-plan`.
- Dry-run mode serializes the same persisted review-adjustment rows as the writer-result JSON path but returns before calling the short-term-plan writer.
- `--dry-run --json` emits `quality_boundary: deterministic_review_adjust_plan_preview`, `dry_run: true`, the target short-term plan path, adjustment count, and preview review-adjustment rows.
- Prose dry-run output reports how many review adjustments would be written and lists the preview rows.
- Corrupt `learning_state.json` handling remains clean in dry-run mode and does not rewrite the short-term plan.
- Safety boundary: v0.49 review adjust-plan dry-run is a no-write preview. It is not a writer, scheduler, exercise generator, tutor, report refresh, LLM call, plan approval, or learning-state mutation.

Verification evidence:
  - `python -m unittest tests.test_learning_plan` failed during RED because `review adjust-plan --dry-run` was not registered.
  - `python -m unittest tests.test_learning_plan` passed with 17 tests OK after implementation.
  - `python -m unittest tests.test_learning_plan tests.test_state_eval tests.test_status_quality_summary tests.test_learning_queue tests.test_reports` passed with 166 tests OK.
  - `python -m unittest discover -s tests` passed with 458 tests OK and 1 skipped.
  - `python -m compileall socrates` passed.
  - `git diff --check` passed.
  - `powershell -ExecutionPolicy Bypass -File scripts\check.ps1` passed with 458 tests OK and 1 skipped.
  - `bash scripts/check.sh` passed with 458 tests OK and 1 skipped; WSL emitted localhost text and a `scripts/check.ps1` line-ending warning, but the script exit code was 0.

## v0.50 Review resolve JSON alpha

- Added `--json` to `python -m socrates review resolve`.
- JSON output still calls `resolve_active_misconceptions_for_concept(...)`, so it performs the same learning-state resolver mutation and mistake-bank append as prose mode.
- The payload records `schema_version`, `quality_boundary: deterministic_misconception_resolver`, `project`, `concept`, resolved count, and structured resolved-misconception rows captured before mutation while serialized with post-action `resolved` status.
- No-match JSON mode returns an empty resolved row list without rewriting `learning_state.json` or appending the mistake bank.
- Corrupt `learning_state.json` handling remains clean in JSON mode and does not append the mistake bank.
- Safety boundary: v0.50 review resolve JSON is a writer-result payload. It is not a dry-run, read-only ledger, note generator, tutor, score, report refresh, LLM call, or extra learning-state truth beyond the existing resolver write.

Verification evidence:
  - `python -m unittest tests.test_state_eval` failed during RED because `review resolve --json` was not registered.
  - `python -m unittest tests.test_state_eval` passed with 59 tests OK after implementation.
  - `python -m unittest tests.test_state_eval tests.test_status_quality_summary tests.test_learning_queue tests.test_reports` passed with 152 tests OK.
  - `python -m unittest discover -s tests` passed with 461 tests OK and 1 skipped.
  - `python -m compileall socrates` passed.
  - `git diff --check` passed.
  - `powershell -ExecutionPolicy Bypass -File scripts\check.ps1` passed with 461 tests OK and 1 skipped.
  - `bash scripts/check.sh` passed with 461 tests OK and 1 skipped; WSL emitted localhost text and a `scripts/check.ps1` line-ending warning, but the script exit code was 0.

## v0.51 Review resolve dry-run alpha

- Added `--dry-run` to `python -m socrates review resolve`.
- Dry-run mode reuses the same active misconception row selection as the resolver writer and returns before calling `resolve_active_misconceptions_for_concept(...)`.
- `--dry-run --json` emits `quality_boundary: deterministic_misconception_resolver_preview`, `dry_run: true`, concept metadata, resolved count, and would-be resolved-misconception rows.
- Prose dry-run output reports how many active misconceptions would be resolved and lists the preview rows.
- No-match dry-run JSON returns an empty resolved row list without rewriting `learning_state.json` or appending the mistake bank.
- Corrupt `learning_state.json` handling remains clean in dry-run mode and does not append the mistake bank.
- Safety boundary: v0.51 review resolve dry-run is a no-write preview. It is not a writer, read-only ledger, note generator, tutor, score, report refresh, LLM call, or learning-state mutation.

Verification evidence:
  - `python -m unittest tests.test_state_eval` failed during RED because `review resolve --dry-run` was not registered.
  - `python -m unittest tests.test_state_eval` passed with 63 tests OK after implementation.
  - `python -m unittest tests.test_state_eval tests.test_status_quality_summary tests.test_learning_queue tests.test_reports` passed with 156 tests OK.
  - `python -m unittest discover -s tests` passed with 465 tests OK and 1 skipped.
  - `python -m compileall socrates` passed.
  - `git diff --check` passed.
  - `powershell -ExecutionPolicy Bypass -File scripts\check.ps1` passed with 465 tests OK and 1 skipped.
  - `bash scripts/check.sh` passed with 465 tests OK and 1 skipped; WSL emitted localhost text and a `scripts/check.ps1` line-ending warning, but the script exit code was 0.

## v0.52 Dashboard JSON alpha

- Added `--json` to `python -m socrates dashboard`.
- JSON mode emits the same read-only dashboard evidence as the Markdown dashboard without parsing the rendered Markdown.
- The payload records `schema_version`, `quality_boundary: deterministic_study_dashboard`, `project`, `root`, `snapshot`, `action_summary`, `top_priority_actions`, and `report_health`.
- `snapshot` includes workflow action count, closeout/regression/report-history status, report-history snapshot count, study-brief freshness fields, and report health counts.
- `action_summary` exposes completion state, open actions, blockers, continue-learning count, human-review count, and first next action.
- Markdown dashboard rendering now uses the same structured payload, preserving one deterministic evidence path for wrappers and humans.
- Safety boundary: v0.52 dashboard JSON is a read-only operator view. It is not a writer, readiness gate, report refresh, repair runner, LLM call, score, tutor, prediction, approval, or project-state mutation.

Verification evidence:
  - `python -m unittest tests.test_dashboard` failed during RED because `dashboard --json` was not registered.
  - `python -m unittest tests.test_dashboard` passed with 4 tests OK after implementation.
  - `python -m unittest tests.test_dashboard tests.test_study_brief tests.test_status_quality_summary tests.test_learning_queue tests.test_reports` passed with 108 tests OK.
  - `python -m unittest discover -s tests` passed with 467 tests OK and 1 skipped.
  - `python -m compileall socrates` passed.
  - `git diff --check` passed.
  - `powershell -ExecutionPolicy Bypass -File scripts\check.ps1` passed with 467 tests OK and 1 skipped.
  - `bash scripts/check.sh` passed with 467 tests OK and 1 skipped; WSL emitted localhost text and a `scripts/check.ps1` line-ending warning, but the script exit code was 0.

## v0.53 Queue JSON alpha

- Added `--json` to `python -m socrates queue`.
- JSON mode emits the same read-only learning queue evidence as Markdown queue output without parsing the rendered Markdown.
- The payload records `schema_version`, `quality_boundary: deterministic_learning_queue`, `project`, `root`, `section`, `action_summary`, and structured `sections`.
- `action_summary` exposes completion state, open actions, blockers, continue-learning count, human-review count, and first next action.
- `sections` preserves deterministic queue ordering and exposes section id, title, count, and item rows with `item_id`, `path`, and `detail`.
- Section-filtered JSON preserves existing queue section selection; `--section priority --json` emits only priority rows, while `--section summary --json` emits no section rows.
- JSON mode is covered by project file-tree snapshots to prove the reader does not write project artifacts.
- Safety boundary: v0.53 queue JSON is a read-only action-queue view. It is not a writer, command runner, repair runner, readiness gate, report refresh, LLM call, score, tutor, prediction, approval, or project-state mutation.

Verification evidence:
  - `python -m unittest tests.test_learning_queue` failed during RED because `queue --json` was not registered.
  - `python -m unittest tests.test_learning_queue` passed with 27 tests OK after implementation.
  - `python -m unittest tests.test_learning_queue tests.test_dashboard tests.test_study_brief tests.test_status_quality_summary tests.test_reports` passed with 110 tests OK.
  - `python -m unittest discover -s tests` passed with 469 tests OK and 1 skipped.
  - `python -m compileall socrates` passed.
  - `git diff --check` passed.
  - `powershell -ExecutionPolicy Bypass -File scripts\check.ps1` passed with 469 tests OK and 1 skipped.
  - `bash scripts/check.sh` passed with 469 tests OK and 1 skipped; WSL emitted localhost text and a `scripts/check.ps1` line-ending warning, but the script exit code was 0.

## v0.54 Brief status JSON alpha

- Added `--json` to `python -m socrates brief status`.
- JSON mode emits the same manifest-backed study-brief freshness evidence as prose `brief status` without parsing the rendered CLI text.
- The payload records `schema_version`, `quality_boundary: deterministic_study_brief_status`, `project`, `root`, `study_brief`, `study_brief_path`, `recorded_next_action`, and `current_next_action`.
- Missing and current brief cases are covered by project file-tree snapshots to prove the reader does not write project artifacts.
- Existing prose `brief status` output remains the human-facing status view.
- Safety boundary: v0.54 brief status JSON is a read-only study-brief freshness view. It is not a generator, writer, readiness gate, report refresh, repair runner, LLM call, score, tutor, prediction, approval, or project-state mutation.

Verification evidence:
  - `python -m unittest tests.test_study_brief` failed during RED because `brief status --json` was not registered.
  - `python -m unittest tests.test_study_brief` passed with 13 tests OK after implementation.
  - `python -m unittest tests.test_study_brief tests.test_dashboard tests.test_status_quality_summary tests.test_learning_queue tests.test_reports` passed with 112 tests OK.
  - `python -m unittest discover -s tests` passed with 471 tests OK and 1 skipped.
  - `python -m compileall socrates` passed.
  - `git diff --check` passed.
  - `powershell -ExecutionPolicy Bypass -File scripts\check.ps1` passed with 471 tests OK and 1 skipped.
  - `bash scripts/check.sh` passed with 471 tests OK and 1 skipped; WSL emitted localhost text and a `scripts/check.ps1` line-ending warning, but the script exit code was 0.

## v0.55 Status JSON alpha

- Added `--json` to `python -m socrates status`.
- JSON mode emits the same central deterministic project status evidence as prose `status` without parsing rendered CLI text.
- The payload records `schema_version`, `quality_boundary: deterministic_project_status`, project/root metadata, current phase, counts, Reference KB status, report history, study brief freshness, queue summary, quality checks, session/workflow status, benchmark status, and misconception counts.
- Prose `status` now renders from the structured payload so human-facing and wrapper-facing status use one evidence path.
- Missing/not-run and invalid manifest semantics remain conservative and match prose status.
- Safety boundary: v0.55 status JSON is a read-only project-status view. It is not a writer, repair runner, readiness-gate expansion, report refresh, LLM call, score, tutor, prediction, approval, or project-state mutation.

Verification evidence:
  - `python -m unittest tests.test_status_quality_summary` failed during RED because `status --json` was not registered.
  - `python -m unittest tests.test_status_quality_summary` passed with 26 tests OK after implementation.
  - `python -m unittest tests.test_status_quality_summary tests.test_dashboard tests.test_study_brief tests.test_learning_queue tests.test_reports` passed with 114 tests OK.
  - `python -m unittest discover -s tests` passed with 473 tests OK, 1 skipped.
  - `python -m compileall socrates` passed.
  - `git diff --check` passed.
  - `powershell -ExecutionPolicy Bypass -File scripts\check.ps1` passed with 473 tests OK, 1 skipped.
  - `bash scripts/check.sh` passed with 473 tests OK, 1 skipped. WSL emitted a localhost/NAT notice and Git warned that `scripts/check.ps1` will be normalized from LF to CRLF when Git next touches it; neither warning failed the check.

## v0.56 MVP lifecycle canary alpha

- Added `python -m socrates lifecycle canary`.
- Added `python -m socrates lifecycle canary --json` for wrapper/plugin-readable canary evidence.
- The canary builds a temporary Group Theory project and runs the minimum composed scenario through initialization, reference curation, Reference KB build, learning plan, two scripted sessions, reviewed/exported notes, generated/validated/banked exercises, an attempted and graded exercise, learning-state/review-schedule updates, weekly/monthly reports, session closeout, multi-session regression, benchmark, and lifecycle audit.
- The JSON payload records `schema_version`, `quality_boundary: deterministic_mvp_lifecycle_canary`, scenario metadata, status, lifecycle counts, artifact counts, audit check rows, and temporary-project cleanup.
- Safety boundary: v0.56 canary is temporary-project-only and deterministic/offline. It is not a user-project writer, live LLM call, UI/plugin, OCR/PDF backend, proof assistant, automatic tutor, or real learner validation.

Verification evidence:
  - `python -m unittest tests.test_lifecycle_canary` failed during RED because `lifecycle canary` was not registered.
  - `python -m unittest tests.test_lifecycle_canary` passed with 2 tests OK after implementation.
  - `python -m unittest tests.test_lifecycle_canary tests.test_lifecycle_audit tests.test_v02_cli_flow tests.test_v04_exercise_flow tests.test_status_quality_summary` passed with 62 tests OK.
  - `python -m unittest discover -s tests` passed with 475 tests OK and 1 skipped.
  - `python -m compileall socrates` passed.
  - `git diff --check` passed.
  - `powershell -ExecutionPolicy Bypass -File scripts\check.ps1` passed with 475 tests OK and 1 skipped.
  - `bash scripts/check.sh` passed with 475 tests OK and 1 skipped. WSL emitted a localhost/NAT notice and Git warned that `scripts/check.ps1` will be normalized from LF to CRLF when Git next touches it; neither warning failed the check.

## v0.57 Canary artifacts alpha

- Added `--artifacts <dir>` to `python -m socrates lifecycle canary`.
- Artifact mode copies the generated temporary canary project into `<dir>/project` and writes `<dir>/canary_report.json`.
- The JSON payload now includes `artifact_bundle` metadata with `written`, `root`, `project_path`, and `report_path`.
- `canary_report.json` records the final payload after the temporary project has been cleaned, so operators can audit both workflow evidence and cleanup.
- Non-empty artifact directories fail with exit code 2 instead of overwriting user files.
- Safety boundary: v0.57 artifact bundles are explicit inspectability outputs. They are not user-project runners, hidden writers, live LLM calls, UI/plugins, OCR/PDF backends, proof assistants, automatic tutors, or real learner validation.

Verification evidence:
  - `python -m unittest tests.test_lifecycle_canary` failed during RED because `--artifacts` was not registered.
  - `python -m unittest tests.test_lifecycle_canary` passed with 4 tests OK after implementation.
  - `python -m unittest tests.test_lifecycle_canary tests.test_lifecycle_audit tests.test_v02_cli_flow tests.test_v04_exercise_flow tests.test_status_quality_summary` passed with 64 tests OK.
  - `python -m unittest discover -s tests` passed with 477 tests OK and 1 skipped.
  - `python -m compileall socrates` passed.
  - `git diff --check` passed.
  - `powershell -ExecutionPolicy Bypass -File scripts\check.ps1` passed with 477 tests OK and 1 skipped.
  - `bash scripts/check.sh` passed with 477 tests OK and 1 skipped. WSL emitted a localhost/NAT notice and Git warned that `scripts/check.ps1` will be normalized from LF to CRLF when Git next touches it; neither warning failed the check.

## v0.58 Returning learner canary evidence alpha

- The MVP lifecycle canary now generates a study brief after the deterministic lifecycle audit and before optional artifact bundle copy.
- The JSON payload now includes `returning_learner` evidence from existing dashboard and resume readers: `study_brief`, `dashboard_study_brief`, `resume_state`, `recommended_command`, `current_next_action`, and `study_brief_path`.
- Artifact counts now include generated study brief and study brief manifest counts.
- Artifact bundles now include `07_exports/briefs/study_brief.md` and `07_exports/briefs/study_brief_manifest.json` inside the copied canary project.
- Safety boundary: v0.58 returning-learner canary evidence is deterministic/offline temporary-project re-entry evidence. It is not a UI/plugin, autonomous tutor, live LLM call, proof assistant, real learner outcome validation, or user-project mutation.

Verification evidence:
  - `python -m unittest tests.test_lifecycle_canary` failed during RED because `artifacts.study_briefs` was missing and the copied artifact bundle did not contain `study_brief.md`.
  - `python -m unittest tests.test_lifecycle_canary` passed with 4 tests OK after implementation.
  - `python -m unittest tests.test_lifecycle_canary tests.test_lifecycle_audit tests.test_v02_cli_flow tests.test_v04_exercise_flow tests.test_status_quality_summary` passed with 64 tests OK.
  - `python -m unittest discover -s tests` passed with 477 tests OK and 1 skipped.
  - `python -m compileall socrates` passed.
...
## v0.59 Product readiness audit alpha

- Added `python -m socrates product readiness` for a deterministic repo-level map from the final v1.0 goal to current capability evidence and remaining gaps.
- Added `python -m socrates product readiness --json` with `quality_boundary: deterministic_product_readiness_audit`, `product_goal: v1.0_math_learning_agent`, `current_version: v0.59-alpha`, and `overall_status: not_v1_ready`.
- The audit exposes ten final-goal capability rows: project initialization, reference discovery/management, literature cleaning/structuring, learning-plan design, guided teaching, personal KB capture, exercise generation, learning-state modeling, misconception bank, and verification/evaluation.
- The JSON payload includes the required next-major-lane decision point with `ocr_pdf_backend`, `autonomous_llm_tutoring`, `ui_plugin_surface`, and `real_long_term_validation` options.
- Safety boundary: v0.59 product readiness is a deterministic repo-level capability/gap map. It is not a v1.0 completion proof, learner-project inspection, project mutation, live LLM call, OCR/PDF backend, UI/plugin, or real learner validation.

Verification evidence:
  - `python -m unittest tests.test_product_readiness` failed during RED because `product` was not a registered top-level command.
  - `python -m unittest tests.test_product_readiness` passed with 2 tests OK after implementation.
  - `python -m unittest tests.test_product_readiness tests.test_lifecycle_canary tests.test_status_quality_summary` passed with 32 tests OK.
  - `python -m unittest discover -s tests` passed with 479 tests OK and 1 skipped.
  - `python -m compileall socrates` passed.
  - `git diff --check` passed.
  - `powershell -ExecutionPolicy Bypass -File scripts\check.ps1` passed with 479 tests OK and 1 skipped.
  - `bash scripts/check.sh` passed with 479 tests OK and 1 skipped. WSL emitted a localhost/NAT notice and Git warned that `scripts/check.ps1` will be normalized from LF to CRLF when Git next touches it; neither warning failed the check.

## v0.60 Real-use validation runbook

- Selected the v0.59 `real_long_term_validation` decision lane instead of immediately implementing OCR/PDF backend, autonomous LLM tutoring, or UI/plugin work.
- Added `docs/superpowers/plans/2026-06-05-v60-real-use-validation-runbook.md` as the operator-facing runbook for one real learner workflow.
- The runbook covers setup, source import, curation, Reference KB build, learning plan, deterministic scripted tutoring, status/dashboard/queue/brief/resume inspection, optional review/closeout/regression/report commands, artifact retention, and a feedback template.
- Safety boundary: v0.60 is a human-run validation pause over the existing v0.59 prototype. It is not a new feature layer, v1.0 completion proof, autonomous tutor, OCR/PDF backend, UI/plugin surface, or permission to continue product implementation before Leo returns feedback.

Verification evidence:
  - `git diff --check` passed.
  - `python -m unittest tests.test_product_readiness` passed with 2 tests OK.
  - `python -m socrates product readiness --json` passed and still reports `current_version: v0.59-alpha`, `overall_status: not_v1_ready`, and one next-major-lane decision point.
  - `powershell -ExecutionPolicy Bypass -File .\tools\lint.ps1` passed in `D:\llmwiki`.
  - The real-use validation cycle itself is intentionally not run by Codex; Leo will run it and return feedback before v0.61 planning.
...
- v1.0：仍缺少完整产品体验、真实长期使用验证、可靠自动组题、阶段性学习报告打磨、UI/插件层和更多真实使用场景回归。

按能力完成度估算，当前约完成最终目标的 60%-70%。剩余 30%-40% 的主要风险不在单个函数，而在长期工作流的稳定性、集成一致性、用户体验和真实数学学习质量。

## 下一步建议

1. 暂停新功能开发；Leo 先按 `docs/superpowers/plans/2026-06-05-v60-real-use-validation-runbook.md` 跑一轮真实学习 workflow。
2. Leo 返回 runbook feedback template 后，再把反馈转成 v0.61 plan。
3. v0.61 的 lane 应从真实摩擦决定：OCR/PDF backend、UI/plugin surface、autonomous LLM tutoring，或 real-use validation round 2。
4. 在反馈回来前，不要继续实现 OCR/PDF、LLM 自主教学、UI/plugin 或新增 workflow feature。
