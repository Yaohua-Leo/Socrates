...
class ExerciseQualityTests(unittest.TestCase):
    ...
    def test_exercise_quality_requires_nonempty_statement_or_review_prompt(self) -> None:
        from socrates.quality import exercise_quality_issues

        with tempfile.TemporaryDirectory() as temp_dir:
            project = create_project(ProjectSpec(topic="Group Theory", path=Path(temp_dir) / "p"))
            exercise = project / "05_exercises" / "generated" / "empty_statement.md"
            exercise.write_text(
                "---\n"
                "status: draft\n"
                "review_status: needs_review\n"
            ...
            self.assertIn("missing statement or review prompt", issues)
    ...
    def test_exercise_quality_requires_difficulty_metadata(self) -> None:
        from socrates.quality import exercise_quality_issues

        with tempfile.TemporaryDirectory() as temp_dir:
            project = create_project(ProjectSpec(topic="Group Theory", path=Path(temp_dir) / "p"))
            exercise = project / "05_exercises" / "generated" / "missing_difficulty.md"
            exercise.write_text(
                "---\n"
                "status: draft\n"
                "review_status: needs_review\n"
            ...
            self.assertIn("missing frontmatter field difficulty", issues)

    def test_exercise_quality_requires_concepts_and_prerequisites_sections(self) -> None:
        from socrates.quality import exercise_quality_issues

        with tempfile.TemporaryDirectory() as temp_dir:
            project = create_project(ProjectSpec(topic="Group Theory", path=Path(temp_dir) / "p"))
            exercise = project / "05_exercises" / "generated" / "missing_schema_sections.md"
            exercise.write_text(
                "---\n"
                "status: draft\n"
                "review_status: needs_review\n"
            ...
            self.assertIn("missing section Concepts", issues)
            self.assertIn("missing section Prerequisites", issues)

    def test_exercise_quality_rejects_invalid_difficulty_value(self) -> None:
        from socrates.quality import exercise_quality_issues

        with tempfile.TemporaryDirectory() as temp_dir:
            project = create_project(ProjectSpec(topic="Group Theory", path=Path(temp_dir) / "p"))
            exercise = project / "05_exercises" / "generated" / "invalid_difficulty.md"
            exercise.write_text(
                "---\n"
                "status: draft\n"
                "review_status: needs_review\n"
            ...
            self.assertIn("invalid difficulty", issues)

    def test_exercise_quality_requires_generated_exercise_source_id(self) -> None:
        from socrates.quality import exercise_quality_issues

        with tempfile.TemporaryDirectory() as temp_dir:
            project = create_project(ProjectSpec(topic="Group Theory", path=Path(temp_dir) / "p"))
            exercise = project / "05_exercises" / "generated" / "missing_source.md"
            exercise.write_text(
                "---\n"
                "status: draft\n"
                "review_status: needs_review\n"
    ...
    def test_exercise_quality_rejects_invalid_frontmatter_lifecycle_values(self) -> None:
        from socrates.quality import exercise_quality_issues

        with tempfile.TemporaryDirectory() as temp_dir:
            project = create_project(ProjectSpec(topic="Group Theory", path=Path(temp_dir) / "p"))
            exercise = project / "05_exercises" / "generated" / "invalid_lifecycle.md"
            exercise.write_text(
                "---\n"
                "status: published\n"
                "review_status: done\n"
            ...
            self.assertIn("invalid exercise status", issues)
    ...
    def test_exercise_quality_rejects_invalid_targeted_review_priority(self) -> None:
        from socrates.quality import exercise_quality_issues

        with tempfile.TemporaryDirectory() as temp_dir:
            project = create_project(ProjectSpec(topic="Group Theory", path=Path(temp_dir) / "p"))
            exercise = project / "05_exercises" / "generated" / "review_invalid_priority.md"
            exercise.write_text(
                "---\n"
                "status: draft\n"
                "review_status: needs_review\n"
            ...
            self.assertIn("invalid targeted review priority", issues)

    def test_exercise_quality_rejects_invalid_targeted_review_scheduled_for(self) -> None:
        from socrates.quality import exercise_quality_issues

        with tempfile.TemporaryDirectory() as temp_dir:
            project = create_project(ProjectSpec(topic="Group Theory", path=Path(temp_dir) / "p"))
            exercise = project / "05_exercises" / "generated" / "review_invalid_date.md"
            exercise.write_text(
                "---\n"
                "status: draft\n"
                "review_status: needs_review\n"
            ...
            self.assertIn("invalid targeted review scheduled_for", issues)
    ...
    def test_exercise_quality_requires_nonempty_concept_tags(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            project = create_project(ProjectSpec(topic="Group Theory", path=Path(temp_dir) / "p"))
            exercise = project / "05_exercises" / "generated" / "empty_concepts.md"
            exercise.write_text(
                "---\n"
                "status: draft\n"
                "review_status: needs_review\n"
            ...
            self.assertIn("missing concept tags", issues)

    def test_exercise_quality_requires_nonempty_prerequisites(self) -> None:
        from socrates.quality import exercise_quality_issues

        with tempfile.TemporaryDirectory() as temp_dir:
            project = create_project(ProjectSpec(topic="Group Theory", path=Path(temp_dir) / "p"))
            exercise = project / "05_exercises" / "generated" / "empty_prerequisites.md"
            exercise.write_text(
                "---\n"
                "status: draft\n"
                "review_status: needs_review\n"
            ...
            self.assertIn("missing prerequisites", issues)

    def test_exercise_quality_requires_nonempty_target_training_point(self) -> None:
        from socrates.quality import exercise_quality_issues

        with tempfile.TemporaryDirectory() as temp_dir:
            project = create_project(ProjectSpec(topic="Group Theory", path=Path(temp_dir) / "p"))
            exercise = project / "05_exercises" / "generated" / "empty_target_training_point.md"
            exercise.write_text(
                "---\n"
                "status: draft\n"
                "review_status: needs_review\n"
            ...
            self.assertIn("missing target training point", issues)

    def test_exercise_quality_requires_nonempty_common_mistakes(self) -> None:
        from socrates.quality import exercise_quality_issues

        with tempfile.TemporaryDirectory() as temp_dir:
            project = create_project(ProjectSpec(topic="Group Theory", path=Path(temp_dir) / "p"))
            exercise = project / "05_exercises" / "generated" / "empty_common_mistakes.md"
            exercise.write_text(
                "---\n"
                "status: draft\n"
                "review_status: needs_review\n"
            ...
            self.assertIn("missing common mistakes", issues)
