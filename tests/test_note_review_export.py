...
class NoteReviewExportTests(unittest.TestCase):
    ...
    def test_export_reviewed_notes_resolves_heading_wikilinks_for_backlinks(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            project = create_project(ProjectSpec(topic="Group Theory", path=Path(temp_dir) / "p"))
            definitions = project / "04_atomic_notes" / "definitions"
            normal_note = definitions / "normal_subgroup.md"
            ...
            quotient_note.write_text(
                ...
                "---\n\n"
                "# Quotient Group\n\n"
                "Compare coset multiplication with "
                "[[Normal Subgroup#Key Examples|normal subgroup examples]].\n\n"
                "## Key Examples\n\n"
                "- Z / nZ.\n\n"
                "## Non-Examples\n\n"
                "- Cosets of a non-normal subgroup do not form a group.\n\n"
    ...
    def test_missing_obsidian_export_file_marks_reviewed_note_pending_again(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            review_atomic_note(project, "normal_subgroup")
            export_reviewed_notes_to_obsidian(project)
            (project / "07_exports" / "obsidian" / "normal_subgroup.md").unlink()
            ...
            self.assertNotIn("normal_subgroup | exported", exported_notes.stdout)
