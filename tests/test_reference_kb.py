...
from socrates.kb import build_reference_kb, find_counterexamples, search_reference_kb
...
class ReferenceKbTests(unittest.TestCase):
    ...
    def test_search_reference_kb_matches_explicit_relationship_targets(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            project = create_project(ProjectSpec(topic="Group Theory", path=Path(temp_dir) / "p"))
            curated = project / "01_references" / "curated" / "examples.curated.md"
            curated.write_text(
            ...
            matches = search_reference_kb(project, "normal subgroup")

            self.assertEqual(len(matches), 1)
            self.assertEqual(matches[0]["title"], "Alternating Group In S3")
    ...
    def test_find_counterexamples_returns_matching_counterexample_objects(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            project = create_project(ProjectSpec(topic="Group Theory", path=Path(temp_dir) / "p"))
            ...
            matches = find_counterexamples(project, "normal subgroup")

            self.assertEqual(len(matches), 1)
            self.assertEqual(matches[0]["type"], "counterexample")
            self.assertEqual(matches[0]["title"], "Subgroup That Is Not Normal")

    def test_build_reference_kb_records_explicit_concept_graph_relationships(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            project = create_project(ProjectSpec(topic="Group Theory", path=Path(temp_dir) / "p"))
            ...
            concept_graph = json.loads(
                (project / "06_kb" / "concept_graph.json").read_text(encoding="utf-8")
            )
            edges = concept_graph["edges"]
            self.assertIn(
                {
                    "source": "alternating_group_in_s3",
                    "target": "normal_subgroup",
            ...
            self.assertIn(
                {
                    "source": "transposition_subgroup",
                    "target": "normal_subgroup",
                    "relationship": "counterexample_to",
                },
                edges,
            )
            self.assertIn(
                {
                    "source": "kernel_normality_proof",
                    "target": "kernel_normality",
                    "relationship": "used_in_proof_of",
                },
                edges,
            )
            dependency_graph = json.loads(
                (project / "06_kb" / "dependency_graph.json").read_text(encoding="utf-8")
            )
            self.assertNotIn("example_of", {edge["relationship"] for edge in dependency_graph["edges"]})
